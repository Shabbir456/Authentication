...
const MarketerView = () => {
    ...
    useEffect(() => {
        ...
        const starCountRef = ref(db, 'MarketersProfiles/' + param2);
        getDownloadURL(Ref(storage, 'MarketersProfiles/' + param2)).then((url) => {
            setimageUrl(url)
            console.log("Image URL NEW" + imageUrl)

        })
        getDownloadURL(Ref(storage, 'MarketerTemplates/' + param2 + '/temp1')).then((url) => {
            settemp1(url)
            console.log("Image URL NEW TEMP" + temp1)

        })
        getDownloadURL(Ref(storage, 'MarketerTemplates/' + param2 + '/temp2')).then((url) => {
            settemp2(url)
            console.log("Image URL NEW TEMP" + temp1)

        })
        getDownloadURL(Ref(storage, 'MarketerTemplates/' + param2 + '/temp3')).then((url) => {
            settemp3(url)
            console.log("Image URL NEW TEMP" + temp1)
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.root}>
                ...
                <ScrollView horizontal={true}
                    ...
                    <View style={{ borderColor: '#767676', borderWidth: 0.5,alignItems: 'flex-start', marginTop: 30, backgroundColor: '#DFDFDF', width: '110%', height: '35%', borderRadius: 25 }}>
                        <Text style={{color: '#767676',alignSelf: 'flex-start', fontSize: 18, marginTop: 5, marginLeft: 30, fontWeight: 'bold' }}>{UserName} Portfolio Display: </Text>
            ...
            <View style={{ marginTop: -600, marginLeft: 20, marginRight: 30 }}>
