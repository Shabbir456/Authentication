...
const UserProfileView = () => {
    ...
    useEffect(() => {
        ...
        setguestUid(param2)
        const starCountRef = ref(db, 'UserProfiles/' + param2);
        ...
        getDownloadURL(Ref(storage, 'UserProfiles/' + param2)).then((url) => {
            setimageUrl(url)
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.root}>
                ...
                <View style={{ alignItems: 'flex-start', marginTop: 30, backgroundColor: '#DFDFDF', width: '100%', height: '40%', borderRadius: 25, borderWidth: 1, borderColor: '#C4C4C4' }}>
                    ...
                    <Text style={{ marginTop: 30, marginLeft: 10, color: '#818380' }}>{UserName} business logo:</Text>
                    <Avatar.Image source={{ uri: imageUrl === '' ? 'https://as1.ftcdn.net/v2/jpg/02/25/63/26/1000_F_225632636_1YVaNrZsCkNYEAUeGXciCPm7xQCXibTa.jpg' : imageUrl }} style={{  alignItems: 'center', justifyContent: 'center' }}
                            size={80}
                        />
